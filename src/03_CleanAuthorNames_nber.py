...
OUTPUT_AUTHOR_CSV = OUTPUT_DIR / "JEL_Training_Data_NBER_AuthorRows.csv"

NBER_AUTHOR_COLUMN = "nber_authors"
NBER_INSTITUTION_COLUMN = "nber_author_institutions"
...
def main() -> None:
    ...
    author_rows = build_nber_author_rows(data)

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    author_rows.to_csv(OUTPUT_AUTHOR_CSV, index=False)

    print("NBER author-name cleaning summary:")
    print(f"  Input CSV: {INPUT_CSV}")
    print(f"  Input rows: {len(data)}")
    print(f"  Rows with nonblank nber_authors: {count_nonblank(data, NBER_AUTHOR_COLUMN)}")
    print(f"  Rows with nonblank nber_author_institutions: {count_nonblank(data, NBER_INSTITUTION_COLUMN)}")
    print(f"  Author-level output CSV: {OUTPUT_AUTHOR_CSV}")
    print(f"  Author-level rows: {len(author_rows)}")
    print(f"  Unique cleaned NBER author names: {author_rows['nber_author_name_clean'].nunique()}")
    print(f"  Unique cleaned ASCII NBER author names: {author_rows['nber_author_name_ascii'].nunique()}")


def build_nber_author_rows(data: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for _, record in data.iterrows():
        authors = split_author_cell(record.get(NBER_AUTHOR_COLUMN, ""))

        for position, author_name_raw in enumerate(authors, start=1):
            author_name_clean = clean_nber_author_name(author_name_raw)
            ...
            rows.append(
                {
                    ...
                    "nber_author_sequence": position,
                    "nber_author_name_raw": author_name_raw,
                    "nber_author_name_clean": author_name_clean,
                    "nber_author_name_ascii": author_name_ascii,
                    "nber_author_first_name": first_name,
                    "nber_author_first_name_length": len(first_name),
                    "nber_author_last_name": last_name,
                    "nber_author_last_name_length": len(last_name),
                }
            )
...
def clean_nber_author_name(name: object) -> str:
